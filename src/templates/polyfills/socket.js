    ...
    class WebsimSocket {
        constructor() {
            ...
            if (window.websimSocketInstance) {
                console.log("[WebSim] Socket already instantiated, returning singleton.");
        ...
        async initialize() {
            console.log("[WebSim] initialize() called");
            try {
        ...
        _handleMessage(msg) {
            ...
            if (type === '_ws_presence') {
                ...
                if (user) {
                    ...
                    // Always update peer info if provided, in case of profile changes
                    this.peers[clientId] = {
                        id: clientId,
                        username: user.username,
                        avatarUrl: avatar,
                        avatar_url: avatar // Consistency for snake_case access
                    };
                }
        ...
        async _announceJoin() {
            console.log("[WebSim] _announceJoin: Preparing to announce presence...");
            
            // Wait for identity (Event driven with timeout to avoid race condition)
            if (!window._currentUser) {
                console.log("[WebSim] _announceJoin: User identity not ready, waiting for GAMEDATA_READY...");
                await new Promise(resolve => {
                    let resolved = false;
                    const onReady = () => {
                        if(resolved) return;
                        resolved = true;
                        console.log("[WebSim] _announceJoin: GAMEDATA_READY received.");
                        window.removeEventListener('GAMEDATA_READY', onReady);
                        resolve();
                    };
                    window.addEventListener('GAMEDATA_READY', onReady);
                    
                    // Fallback timeout (5s) - don't block game forever
                    setTimeout(() => {
                        if(!resolved) {
                            resolved = true;
                            console.warn("[WebSim] _announceJoin: Timed out waiting for identity. Proceeding as Guest.");
                            window.removeEventListener('GAMEDATA_READY', onReady);
                            resolve();
                        }
                    }, 5000);
        ...
        static updateIdentity(user) {
            console.log("[WebSim] Updating Identity:", user?.username);
            // Ensure we store a safe URL in the shared state
            if (user) {
                const safeAvatar = sanitizeAvatar(user.avatar_url, user.username);
                user.avatar_url = safeAvatar;
                // Polyfill camelCase/snakeCase for consistency
            ...
            if (inst && inst.peers[inst.clientId]) {
                // Update internal peer record
                inst.peers[inst.clientId].username = user.username;
                inst.peers[inst.clientId].avatarUrl = user.avatar_url;
                inst.peers[inst.clientId].avatar_url = user.avatar_url;
                
                // Trigger broadcast with new info
                console.log("[WebSim] Broadcasting updated identity...");
                inst.updatePresence({}); 
            }
        }
    }
        ...
        init: async () => {
            ...
            try {
                ...
                if (data.dbData) {
                    ...
                    // Note: window._currentUser is set via updateIdentity below for consistency
                    
                    if (window.WebsimSocket) {
                        window.WebsimSocket.updateIdentity(data.user);
                    } else {
                        // Fallback if socket class not ready yet
